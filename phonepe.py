...
top_user= pd.DataFrame(table9, columns=("States", "Years", "Quarter", "Pincodes",
                    ...
                    #========== / TITLE SETUP / =============#
icon = Image.open("/Users/priyanka/Desktop/Visual Studio Workbench/phpe_pulse/phpe_logo.png")

                    #=========== / FUNCTION FOR PLOTS / ================#

# 1. Transaction_amount_count_year
...
# 2. Transaction_amount_count_Year_Quarter
...
# 3. Aggregated transaction for types   
def Aggre_Tran_Transaction_type(df, state):
    ...
    with col1:
        fig_pie_1= px.pie(data_frame= tacyg, names= "Transaction_type", values= "Transaction_amount",
                            width= 600, title= f"{state.upper()} FOR {tacy['Years'].min()} TRANSACTION AMOUNT", hole= 0.5)
        st.plotly_chart(fig_pie_1)

    with col2:
        fig_pie_2= px.pie(data_frame= tacyg, names= "Transaction_type", values= "Transaction_count",
                            width= 600, title= f"{state.upper()} FOR {tacy['Years'].min()} TRANSACTION COUNT", hole= 0.5)
        st.plotly_chart(fig_pie_2)

    return tacy

# 4. Aggregated user plots
# 4.1 Aggre_user_Analysis_1
...
def top_chart_registered_users(table_name):
    ...
    st.plotly_chart(fig_amount_3)
                                  ...
                                  #============= / STREAMLIT PART / ===============#
st.set_page_config(page_title="PhonePe Pulse", page_icon=icon, layout='wide')

with st.sidebar:
    select = option_menu("Main Menu",["HOME", "EXPLORATION","TOP CHARTS"])
...
if select == "HOME":
    ...
    with col2:
        st.image(Image.open(r"/Users/priyanka/Desktop/Visual Studio Workbench/phpe_pulse/phonepe-to-compete-google-with-launch-of.jpeg"),width= 600)

    col3,col4= st.columns(2)
    
    with col3:
        st.image(Image.open(r"/Users/priyanka/Desktop/Visual Studio Workbench/phpe_pulse/PHONEPEIMAGE.jpeg"),width=600)
    ...
    with col6:
        st.image(Image.open(r"/Users/priyanka/Desktop/Visual Studio Workbench/phpe_pulse/PHONEPE_IMG_2.jpeg"),width= 600)


# EXPLORATION
elif select == "EXPLORATION":
    col1, col2 = st.columns(2)
    with col1:
        # Display an image
        st.image("phonpe pulse.png")

    with col2:
        # Title
        st.header(':violet[ Data Visualization ]')
        st.write('**(Note)**:- This data between **2018** to **2024** in **INDIA**')
    ...
    with tab1:
        ...
        if method == "Aggregated Transaction":
            ...
            with col1:
                years = st.selectbox("Select the Year", options=sorted(Aggre_transaction["Years"].unique()))
            tac_Y1 = Transaction_amount_count_Y(Aggre_transaction, years)

            with col2:
                states= st.selectbox("Select the States", options=sorted(tac_Y1["States"].unique()))
            Aggre_Tran_Transaction_type(tac_Y1,states)

            with col3:
                quarters = st.selectbox("Select the Quarter", options=tac_Y1["Quarter"].unique())
            tac_Y1_Q = Transaction_amount_count_Y_Q(tac_Y1, quarters)


        elif method == "Aggregated User":
            col1,col2,col3 = st.columns(3)
            with col1:
                years = st.selectbox("Select the Year", options=sorted(Aggre_user["Years"].unique()))
            Aggre_user_Y = Aggre_user_plot_1(Aggre_user, years)

            with col2:
                quarters = st.selectbox("Select the Quarter", options=Aggre_user_Y["Quarter"].unique())
            Aggre_user_Y_Q= Aggre_user_plot_2(Aggre_user_Y, quarters)

            with col3:
                states= st.selectbox("Select the States", options=sorted(Aggre_user_Y_Q["States"].unique()))
    ...
    with tab2:
        ...
        if method_2 == "Map Transaction":
            col1,col2,col3 = st.columns(3)
            with col1:
                years = st.selectbox("Select the Year for Map", options=sorted(map_transaction["Years"].unique()))
            map_tran_tac_Y = Transaction_amount_count_Y(map_transaction, years)

            with col2:
                states= st.selectbox("Select the States for Map", options=sorted(map_tran_tac_Y["States"].unique()))
            Map_trans_District(map_tran_tac_Y,states)

            with col3:
                quarters = st.selectbox("Select the Quarter for Map", options=map_tran_tac_Y["Quarter"].unique())
            ...
            with col1:
                years = st.selectbox("Select The Year for Map", options=sorted(map_user["Years"].unique()))
            map_user_Y = map_user_plot_1(map_user, years)

            with col2:
                quarters = st.selectbox("Select the Quarter for Map", options=map_user_Y["Quarter"].unique())
            map_user_Y_Q = map_user_plot_2(map_user_Y, quarters)

            with col3:
                states = st.selectbox("Select the States for Map", options=sorted(map_user_Y_Q["States"].unique()))
    ...
    with tab3:
        ...
        if method_3 == "Top Transaction":
            ...
            with col1:
                years = st.selectbox("Select The Year for Top", options=sorted(top_transaction["Years"].unique()))
            top_tran_tac_Y = Transaction_amount_count_Y(top_transaction, years)

            with col2:
                states = st.selectbox("Select The State for Top", options=sorted(top_tran_tac_Y["States"].unique()))
            Top_trans_plot_1(top_tran_tac_Y, states)

            with col3:
                quarters = st.selectbox("Select the Quarter for Top", options=top_tran_tac_Y["Quarter"].unique())
            ...
            col1,col2= st.columns(2)
            with col1:
                years = st.selectbox("Select The Year for Top", options=sorted(top_user["Years"].unique()))
            top_user_Y= top_user_plot_1(top_user, years)

            with col2:
                states = st.selectbox("Select The State for Top", options=sorted(top_user_Y["States"].unique()))
